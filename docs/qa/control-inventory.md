...
| BR01 downstream handoff/output consequence mapping | `src/modules/br01/downstream.ts`, `src/modules/output/index.ts`, `src/modules/handoff/index.ts` | Stored BR01 `RoutingDecision` / `ReferralFlag` artifacts are preferred for downstream review/referral posture when present, with explicit request-time BR01 fallback only when stored artifacts are absent. Split-matter, referral, and route-out remain distinct in structural controls, and deterministic arrears-only does not inject extra BR01 caution controls. | `tests/br01-downstream.test.ts` |
...
| BR03 touchpoint classification and freshness-control engine | `src/modules/touchpoints/index.ts`, `src/modules/output/index.ts`, `src/modules/handoff/index.ts` | Touchpoint consumers distinguish `MIRROR`, `MIRROR_WITH_WARNING`, and `DEFER_TO_LIVE_OFFICIAL_FLOW`, plus stale/live/wrong-channel posture, and propagate those controls into output/handoff seams without introducing product-side portal execution. Wrong-channel reroute posture and authenticated-handoff-only posture both override ordinary mirror allowances, and mixed stale/live combinations stay distinct under multi-touchpoint inputs. | `tests/br03-touchpoint-control.test.ts`, `tests/br03-touchpoint-matrix.test.ts`, `tests/output-handoff.framework.test.ts` |
...
| `TOUCHPOINT_STALE` | BR03 touchpoint carry-forward control | Stale touchpoint posture remains cautionary and non-authoritative until refreshed. |
...
| `TOUCHPOINT_LIVE_CONFIRMATION_REQUIRED` | BR03 touchpoint carry-forward control | Live official confirmation is required before relying on volatile touchpoints. |
...
| BR02 downstream workflow/handoff bridge | `src/workflow/arrearsHeroWorkflow.ts`, `src/modules/output/index.ts`, `src/modules/handoff/index.ts`, `src/modules/br02/downstream.ts`, `src/modules/br02/runtimeBridgeThreading.ts` | When notice-readiness is absent, downstream workflow, prep-pack, and handoff consumers derive readiness and posture from nested BR02 `consumerAssessment` with explicit `HARD_STOP`, `NEEDS_REVIEW`, `REVIEW_LED_CAUTION`, and `NEXT_STEP_READY` mapping. RuntimeBridge can be threaded from BR02 assessment call sites via `runtimeBridgeThreading.downstreamInputs`; conservative payment-plan branch codes (`PAYMENT_PLAN_MINIMUM_WINDOW_PENDING`, `PAYMENT_PLAN_REVIEW_REQUIRED`) remain sign-off-gated before bridge threading is widened. | `tests/br02-downstream.test.ts`, `tests/br02-runtime-bridge-threading.test.ts` |
