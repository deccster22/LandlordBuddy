...
| BR01 downstream handoff/output consequence mapping | `src/modules/br01/downstream.ts`, `src/modules/output/index.ts`, `src/modules/handoff/index.ts` | BR01 routing posture drives downstream review/referral behavior through existing structural controls: split-matter triggers review posture, referral/route-out trigger referral-stop posture, and deterministic arrears-only does not inject extra BR01 caution controls. | `tests/br01-downstream.test.ts` |
