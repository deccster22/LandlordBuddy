...
| Mandatory unpaid-rent notice fields | `src/modules/notice-readiness/index.ts` | Missing `arrearsAmount`, `paidToDate`, `noticeNumber`, or `serviceMethod` produces a `hard stop` issue with a field-specific code and keeps `deterministicPass` false. | `tests/notice-readiness.validator.test.ts` |
| Interstate hard stop | `src/modules/notice-readiness/index.ts` | `interstateRouteOut: true` produces `outcome: hard stop`, `routeOut: true`, and `readyForStandardProgression: false`. | `tests/notice-readiness.validator.test.ts` |
