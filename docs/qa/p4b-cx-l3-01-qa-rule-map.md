...
Follow-on timeline combinatorial matrix: `docs/qa/p4b-cx-l3-04-timeline-combinatorial-regression-matrix.md`.
...
| `L3-COV-04` | Stale downgrade from BR03 to output/handoff/renderer surfaces | `covered` | `tests/br03-touchpoint-control.test.ts`, `tests/br03-touchpoint-matrix.test.ts`, `tests/l3-stale-renderer-regression.test.ts`, `tests/l3-timeline-combinatorial-regression.test.ts` | Distinctions (`stale` vs `live-confirmation` vs `wrong-channel`) remain explicit, including mixed and timeline-blocked BR02+BR03 downstream paths. |
| `L3-COV-05` | BR02 stale-state downgrade propagated through downstream output/handoff surfaces | `covered` | Stale warning and stale slowdown downstream bridge scenarios are asserted in `tests/br02-downstream.test.ts`, with baseline semantics anchored in `tests/br02-registry-scaffold.test.ts` and `tests/br02-consumer.test.ts`; mixed stale + touchpoint downstream paths are asserted in `tests/l3-stale-renderer-regression.test.ts` and `tests/l3-timeline-combinatorial-regression.test.ts` | No mismatch found in this pass. |
...
1. Expand from current timeline-blocked combinatorial matrix coverage into broader high-cardinality permutations (for example, additional touched touchpoint sets plus BR02 stale/hearing combinations with authenticated-handoff controls).
