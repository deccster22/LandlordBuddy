...
- `src/modules/br02/runtimeBridge.ts` now exposes a typed runtime bridge contract over the settled BR02 calculators, including explicit segment posture (`DETERMINISTIC_RESULT`, `GUARDED_WARNING_OR_REVIEW`, `OVERRIDE_SENSITIVE_RESULT`, `HARD_STOP`) plus a guarded payment-plan timing branch (`>= 7` business days).
...
- `src/modules/br02/index.ts` now also carries the nested `runtimeBridge` bundle on service-event assessment output so implementation-facing callers can consume one bridge object without flattening guarded seams.
- `src/modules/br02/runtimeBridgeThreading.ts` now adds a call-site threading decision that always carries `br02ConsumerAssessment` and threads `br02RuntimeBridge` only when payment-plan posture is non-conservative or explicit sign-off is accepted. Conservative payment-plan branches remain sign-off-gated before wider bridge-default adoption.
