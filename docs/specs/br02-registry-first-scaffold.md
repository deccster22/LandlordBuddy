...
- The legacy top-level `readyForDeterministicDateHandling` flag is no longer part of the BR02 bridge/workflow comparison path. It remains only as a legacy compatibility field on the service-event assessor shape so older direct consumers can still read the old output, but downstream handoff logic does not use it.
