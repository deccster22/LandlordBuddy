...
3. Output-package records now consume the same source-driven BR04 policy lane. They remain prep-and-handoff records only and do not imply filing, submission, or compliant final disposal behavior.
...
5. Auditability remains explicit. Privacy actions have a dedicated event shape instead of hiding lifecycle changes in generic notes or silent state transitions.
...
- Later BR04 output work can build on the new output-package record factory instead of inventing a second privacy spine.
